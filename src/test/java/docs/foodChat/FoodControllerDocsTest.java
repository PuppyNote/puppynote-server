...
public class FoodControllerDocsTest extends RestDocsSupport {
    ...
    @DisplayName("음식 조회 API - ES 캐시 히트")
    @Test
    void searchFood_cacheHit() throws Exception {
        ...
        FoodListResponse listResponse = FoodListResponse.of(List.of(foodResponse), 0, 1L);
        given(foodChatService.search(any())).willReturn(listResponse);
        ...
        mockMvc.perform(
                ...
                .andDo(document("food-search",
                        ...
                        responseFields(
                                ...
                                fieldWithPath("data.content").type(JsonFieldType.ARRAY).description("음식 정보 목록"),
    ...
    @DisplayName("음식 조회 API - AI 신규 응답")
    @Test
    void searchFood_aiResponse() throws Exception {
        FoodResponse foodResponse = mock(FoodResponse.class);
        ...
        mockMvc.perform(
                        get("/api/v1/foods")
                                .param("question", "포도는 강아지에게 줘도 되나요?")
                                .param("page", "0")
                                .param("size", "10")
                )
                .andDo(print())
                .andExpect(status().isOk())
                .andDo(document("food-search-ai",
                        preprocessRequest(prettyPrint()),
                        preprocessResponse(prettyPrint()),
                        queryParameters(
                                parameterWithName("question").description("음식 관련 질문 (필수)"),
